...
import {
  IResponseStructure,
  IPlace,
  IPreviewResponseStructure,
  IPlacesByCity,
  IPlaceNeedingAttention,
...
import {
  ...
  getPreviewData,
  savePreviewData,
  getUncheckedPlacesByCity as getUncheckedPlacesByCityFromDB,
  saveUncheckedPlacesByCity,
  getPlacesNeedingAttention as getPlacesNeedingAttentionFromDB,
  savePlacesNeedingAttention,
  getSinglePlace,
  saveSinglePlace,
  clearAllData,
...
export const PlaceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [previewData, setPreviewData] =
    useState<IPreviewResponseStructure | null>(null);
  const [uncheckedPlacesByCity, setUncheckedPlacesByCity] =
    useState<IPlacesByCity | null>(null);
  const [placesNeedingAttention, setPlacesNeedingAttention] = useState<
    IPlaceNeedingAttention[] | null
  >(null);
  ...
  useEffect(() => {
    console.log(
      "[PlacesContext] Data state changed, keys:",
      Object.keys(data),
      "unchecked count:",
  ...
  const fetchData = useCallback(async (isAdmin: boolean): Promise<void> => {
    ...
    if (fetchingRefs.current[fetchKey]) {
      console.log(
        "[PlacesContext] A fetch is already in progress for:",
        fetchKey
      );
    ...
    try {
      ...
      console.log(
        "[PlacesContext] API response received with data keys:",
  ...
  const getPreview = useCallback(async (isAdmin: boolean): Promise<void> => {
  ...
  const getUncheckedPlacesByCity = useCallback(
    async (cityName: string): Promise<IPlacesByCity> => {
      console.log(
        `[PlacesContext] getUncheckedPlacesByCity called for "${cityName}"`
      );

      // Check if we already have this data in memory cache
      if (cityDataCache.current[cityName]) {
        console.log(
          `[PlacesContext] Using in-memory cached data for city: ${cityName}`
        );
      ...
      if (requestsMade.current[requestKey]) {
        console.log(
          `[PlacesContext] Request for ${cityName} already made in this session`
        );
        if (uncheckedPlacesByCity) {
          console.log(
            `[PlacesContext] Returning current uncheckedPlacesByCity state for ${cityName}`
          );
      ...
      try {
        console.log(
          `[PlacesContext] Starting fetch for unchecked places in ${cityName}`
        );
        fetchingRefs.current[fetchKey] = true;
        ...
        if (cachedCityData) {
          console.log(
            `[PlacesContext] Using cached data from IndexedDB for ${cityName}`
          );
        ...
        // If no cached data, fetch from API
        console.log(
          `[PlacesContext] No cached data found for ${cityName}, fetching from API`
        );
        const response = await apiClient.get<IPlacesByCity>(
          `/front/getUncheckedPlacesByCity/${encodeURIComponent(cityName)}`
        );
        const responseData = response.data;
        console.log(
          `[PlacesContext] Received ${
        ...
        console.error(
          `[PlacesContext] Error fetching unchecked places for ${cityName}:`,
          error
        );
        return Promise.reject(error);
      } finally {
        fetchingRefs.current[fetchKey] = false;
        console.log(
          `[PlacesContext] getUncheckedPlacesByCity for ${cityName} completed`
        );
      }
    },
  ...
  const getPlacesNeedingAttention = useCallback(async (): Promise<
    IPlaceNeedingAttention[]
  > => {
    ...
    try {
      ...
      // If no cached data, fetch from API
      const response = await apiClient.get<IPlaceNeedingAttention[]>(
        "/front/getAllPlacesNeedingAttention"
      );
  ...
  const findPlaceById = useCallback(
    (placeId: number): IPlace | undefined => {
      ...
      for (const status of [
        ...
        for (const country of Object.keys(data[status])) {
          for (const city of Object.keys(data[status][country])) {
            const cityPlaces = data[status][country][city] as unknown as Record<
              string,
              IPlace[]
            >;
            for (const placeKey of Object.keys(cityPlaces)) {
              const placeArray: IPlace[] = cityPlaces[placeKey];
              const foundPlace = placeArray.find(
                (p: IPlace) => p.place_id === placeId
              );
              if (foundPlace) {
  ...
  const updateSinglePlace = useCallback(
    async (placeId: number): Promise<void> => {
      ...
      try {
            ...
            for (const status of [
              ...
              for (const country of Object.keys(updatedData[status])) {
                for (const city of Object.keys(updatedData[status][country])) {
                  const cityPlaces = updatedData[status][country][
                    city
                  ] as unknown as Record<string, IPlace[]>;
                  for (const placeKey of Object.keys(cityPlaces)) {
                    const placeArray: IPlace[] = cityPlaces[placeKey];
                    const placeIndex = placeArray.findIndex(
                      (p: IPlace) => p.place_id === placeId
                    );
                    if (placeIndex !== -1) {
        ...
        // If no cached data, fetch from API
        const response = await apiClient.get<IPlace>(
          `/front/${placeId}/images`
        );
  ...
  const refreshAllData = useCallback(
    async (isAdmin: boolean): Promise<void> => {
      ...
      try {
        ...
        const previewResponse = await apiClient.get<IPreviewResponseStructure>(
          previewUrl
        );
        ...
        const placesResponse = await apiClient.get<IResponseStructure>(
          placesUrl
        );
        ...
        const attentionResponse = await apiClient.get<IPlaceNeedingAttention[]>(
          "/front/getAllPlacesNeedingAttention"
        );
  ...
  console.log(
    "[PlacesContext] Context rendering with data lengths:",
    Object.keys(data.unchecked).length,
  ...
  const contextValue = useMemo(() => {
    console.log("[PlacesContext] Context value being recreated");
    return {
      data,
      previewData,
      uncheckedPlacesByCity,
