...
const HomePage: React.FC = () => {
  ...
  const [filteredPlaces, setFilteredPlaces] = useState<
    { country: string; city: string; place: IPlace; status: string }[]
  >([]);
  ...
  useEffect(() => {
    console.log(
      "[HomePage] Search useEffect triggered with searchQuery:",
      searchQuery,
      "length:",
      searchQuery.length
    );
    ...
    if (searchQuery && searchQuery.length >= 3) {
      console.log("[HomePage] Setting up 2 second timeout for search");

      const timeoutId = setTimeout(() => {
        console.log(
          "[HomePage] Timeout completed, starting search, setting loading to true"
        );
        setIsLoading(true);
        // If user is searching, load the full data to enable search functionality
        console.log(
          "[HomePage] Calling updatePlaces with admin:",
          user?.admin || false
        );
        updatePlaces(user?.admin || false)
          .then(() => {
            console.log("[HomePage] updatePlaces completed successfully");
            setIsLoading(false);
          })
          .catch((error) => {
            console.log("[HomePage] updatePlaces failed with error:", error);
            setIsLoading(false);
          });
      }, 2000); // 2 second delay
      ...
      return () => {
        console.log("[HomePage] Clearing timeout");
        clearTimeout(timeoutId);
      };
    } else {
      console.log(
        "[HomePage] Search query too short or empty, skipping updatePlaces"
      );
    }
  ...
  useEffect(() => {
    console.log(
      "[HomePage] Filter useEffect triggered with searchQuery:",
      searchQuery,
      "places type:",
      typeof places,
      "places keys:",
      places ? Object.keys(places) : "null"
    );
    if (searchQuery && searchQuery.length >= 3 && places) {
      console.log("[HomePage] Starting to filter places");
      ...
      ["unchecked", "needs_attention", "to_be_deleted"].forEach((status) => {
        for (const country of Object.keys(places[status])) {
          for (const city of Object.keys(places[status][country])) {
            for (const place of Object.values(
              places[status][country][city] || {}
            ).flat()) {
              if (
                (place.place_name &&
                  place.place_name
      ...
      console.log(
        "[HomePage] Found",
        newFilteredPlaces.length,
        "filtered places"
      );
      setFilteredPlaces(newFilteredPlaces);
    } else {
      console.log("[HomePage] Clearing filtered places");
      setFilteredPlaces([]);
    }
  ...
  const calculateTotals = () => {
    ...
    for (const country of Object.keys(previewData.unchecked)) {
      ...
      for (const city of Object.keys(previewData.unchecked[country])) {
        ...
        for (const placeName of Object.keys(
          previewData.unchecked[country][city]
        )) {
          totalPlacesUnchecked +=
            previewData.unchecked[country][city][placeName].place_count;
        }
      }
    }
    ...
    for (const country of Object.keys(previewData.needs_attention)) {
      ...
      for (const city of Object.keys(previewData.needs_attention[country])) {
        uniqueCities.add(`${country}-${city}`);
        for (const placeName of Object.keys(
          previewData.needs_attention[country][city]
        )) {
          totalPlacesNeedsAttention +=
            previewData.needs_attention[country][city][placeName].place_count;
        }
      }
    }
    ...
    for (const country of Object.keys(previewData.to_be_deleted)) {
      ...
      for (const city of Object.keys(previewData.to_be_deleted[country])) {
        uniqueCities.add(`${country}-${city}`);
        for (const placeName of Object.keys(
          previewData.to_be_deleted[country][city]
        )) {
          totalPlacesToBeDeleted +=
            previewData.to_be_deleted[country][city][placeName].place_count;
        }
      }
    }
    ...
    if (user?.admin) {
      for (const country of Object.keys(previewData.checked)) {
        ...
        for (const city of Object.keys(previewData.checked[country])) {
          uniqueCities.add(`${country}-${city}`);
          for (const placeName of Object.keys(
            previewData.checked[country][city]
          )) {
            totalPlacesChecked +=
              previewData.checked[country][city][placeName].place_count;
          }
        }
      }
    }
  ...
  const getCountsForCountry = (countryName: string, viewChecked: boolean) => {
    ...
    const countryData = viewChecked
      ? previewData.checked[countryName]
      : previewData.unchecked[countryName];
  ...
  const getCountsForCity = (
    ...
    const cityData = viewChecked
      ? previewData.checked[countryName]?.[cityName]
      : previewData.unchecked[countryName]?.[cityName];
    let placeCount = 0;
  ...
  if (countryName && cityName && selectedCityPlaces.length > 0) {
    return (
      <PhotoSelectorCity places={selectedCityPlaces} cityName={cityName} />
    );
  }
  ...
  return (
    <div className="container mt-5">
      ...
      <h1 className="mb-4 text-center">
        {viewChecked ? "Lieux traités ✅" : "Lieux à traiter ❌"}
      </h1>
      {user?.admin && (
        <div className="d-flex justify-content-center mb-4">
          ...
          >
            {viewChecked
              ? "Afficher lieux à traiter ❌"
              : "Afficher lieux traités ✅"}
          </button>
        </div>
      )}
      ...
      <div className="d-flex justify-content-center">
        <ul className="list-group mb-4 w-50">
          ...
          <li className="list-group-item">
            <strong>🚨 Lieux nécessitant une attention :</strong>{" "}
            {totalPlacesNeedsAttention}
          </li>
          <li className="list-group-item">
      ...
      <div className="d-flex justify-content-center mb-4">
        <input
          ...
          value={searchQuery}
          onChange={(e) => {
            console.log("[HomePage] Search input changed to:", e.target.value);
        ...
        <div className="d-flex justify-content-center">
          <ul className="list-group w-50">
            {filteredPlaces.map(({ country, city, place, status }) => (
              <li
                key={`${country}-${city}-${place.place_id}`}
      ...
      <div className="row">
        {Object.keys(previewData[viewChecked ? "checked" : "unchecked"]).map(
          (countryName) => {
            const { cityCount, placeCount } = getCountsForCountry(
              countryName,
              viewChecked
            );
            return (
              <div key={countryName} className="col-md-4 mb-4">
                <div className="card shadow-sm">
                  ...
                  <div className="card-body">
                    {Object.keys(
                      previewData[viewChecked ? "checked" : "unchecked"][
                        countryName
                      ]
                    ).map((cityName) => (
